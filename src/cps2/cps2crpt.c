...
static u8 fn(u8 in, const struct sbox *sboxes, u32 key)
{
	int box;
	u8 res = 0;
...
static void expand_1st_key(unsigned int *dstkey, const unsigned int *srckey)
{
...
static void expand_2nd_key(unsigned int *dstkey, const unsigned int *srckey)
{
	static const int bits[96] =
	{
		20, 57, 26, 58, 47, 40,
...
{
	const u8 l0 = BITSWAP8(val, bitsB[7],bitsB[6],bitsB[5],bitsB[4],bitsB[3],bitsB[2],bitsB[1],bitsB[0]);
	const u8 r0 = BITSWAP8(val, bitsA[7],bitsA[6],bitsA[5],bitsA[4],bitsA[3],bitsA[2],bitsA[1],bitsA[0]);

	const u8 l1 = r0;
	const u8 r1 = l0 ^ fn(r0, boxes1, key1);

	const u8 l2 = r1;
	const u8 r2 = l1 ^ fn(r1, boxes2, key2);

	const u8 l3 = r2;
	const u8 r3 = l2 ^ fn(r2, boxes3, key3);

	const u8 l4 = r3;
	const u8 r4 = l3 ^ fn(r3, boxes4, key4);
...
static void cps2_decrypt(const u32 *master_key, u32 upper_limit)
{
	u16 *rom = (u16 *)memory_region_cpu1;
	u32 length = memory_length_user1;
	u16 *dec = (u16 *)memory_region_user1;
	u32 i, percent = 0, prev_percent = 0;
	u32 key1[4];
