...
----------------------------------------------------------------------
キャッシュファイルについて(※1.30で少し変更)

  従来、容量を節約するためZIP圧縮したキャッシュファイルを使用して
  いましたが、1.30からフォルダ(cache/gamename_cache)に展開した
  無圧縮のファイルを読み込めるよう変更しました。

  cahceフォルダにgamename_cache(例:samsho2_cache)という名前のフォルダを
  作成し、romcnv_mvs.exeで作成したキャッシュに含まれるファイルを全て
  展開してください。チェックしていないため、どの程度速くなるかはわかり
  ませんが、大分変化があるのではないかと思います。

  ZIPファイルの展開も少し高速化できそうなので、調整してみます。
